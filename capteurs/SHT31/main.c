...
char msg_lcd[17]; // Buffer pour le texte du LCD (16 chars + \0)
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  // 3. Lecture des données
	      ...
	      {
	          // Alerte si la température dépasse 30°C
	          ...
	          // Affichage ligne 1 (Temp)
	          formatTemp(msg_lcd, sensor.temperature);
	          LCD_SetCursor(&lcd, 0, 0);
	          LCD_EcrireTexte(&lcd, msg_lcd);

	          // Affichage ligne 2 (Humi)
	          formatHumi(msg_lcd, sensor.humidity);
	      ...
	      {
	          LCD_SetCursor(&lcd, 0, 0);
	          LCD_EcrireTexte(&lcd, "Read Error!     ");
	      }

	      HAL_Delay(2000); // Mise à jour toutes les 2s
	      /* USER CODE END WHILE */
  }
    /* USER CODE END WHILE */
...
void formatTemp(char *msg, float temp) {
    ...
    if (t_dec < 0) t_dec *= -1;
    // Format: "Temp: 25.4 C  "
    sprintf(msg, "Temp: %d.%d C  ", t_int, t_dec);
}
...
void formatHumi(char *msg, float humi) {
    ...
    // Format: "Humi: 45.8 %  "
    sprintf(msg, "Humi: %d.%d %%  ", h_int, h_dec);
}
/* USER CODE END 4 */
